...
def get_user_credits(user_id):
    """Get user's credit balance from MongoDB."""
    if users_collection is None:
        return 100  # Default for dev mode
...
def deduct_credits(user_id, feature):
    """Deduct credits for using a premium feature."""
    if users_collection is None:
    ...
    try:
        ...
        if user.get("is_pro", False):
            return {
                "success": True,
                "remaining": user.get("credits", 100),
                "deducted": 0,
                "is_pro": True,
            }
        ...
        if feature == "generate_plan":
            used = user.get("plan_generations_used", 0)
            limit = FREE_LIMITS.get("generate_plan", 10)
            if used < limit:
                # Still within free limit
                users_collection.update_one(
                    {"userId": user_id}, {"$inc": {"plan_generations_used": 1}}
                )
                return {
                    "success": True,
                    "remaining": credits,
        ...
        if feature == "coach_chat":
            used = user.get("coach_chats_used", 0)
            limit = FREE_LIMITS.get("coach_chat", 10)
            if used < limit:
                users_collection.update_one(
                    {"userId": user_id}, {"$inc": {"coach_chats_used": 1}}
                )
                return {
                    "success": True,
                    "remaining": credits,
        ...
        users_collection.update_one(
            {"userId": user_id}, {"$set": {"credits": new_credits}}
        )

        return {"success": True, "remaining": new_credits, "deducted": cost}
...
def get_credits(user_id):
    ...
    user_data = {"credits": credits, "is_pro": False}
    if users_collection:
        try:
            user = users_collection.find_one({"userId": user_id})
            if user:
                user_data["credits"] = user.get("credits", 0)
                user_data["is_pro"] = user.get("is_pro", False)
                user_data["plan_generations_used"] = user.get(
                    "plan_generations_used", 0
                )
                user_data["coach_chats_used"] = user.get("coach_chats_used", 0)
        except:
            pass
...
def deduct_user_credits():
    ...
    if not user_id or not feature:
        return jsonify({"error": "Missing user_id or feature"}), 400

    result = deduct_credits(user_id, feature)
...
def check_feature_access():
    ...
    if users_collection is None:
        return jsonify({"can_access": True, "credits": 100})
        ...
        return jsonify(
            {
                ...
                "credits": credits,
                "cost": cost,
                "needed": cost if not can_access else 0,
            }
        )

    except Exception as e:
        print(f"[Credits] Error checking access: {e}")
        return jsonify({"can_access": True, "credits": 100})
...
def add_credits():
    ...
    if users_collection is None:
        return jsonify({"success": True, "credits": 100 + credits_to_add})
...
@app.route("/api/user/reset-limits/<user_id>", methods=["POST"])
def reset_free_limits(user_id):
    """Reset free usage limits (for testing or after payment)."""
    if users_collection is None:
    ...
    try:
        users_collection.update_one(
            {"userId": user_id},
            {"$set": {"plan_generations_used": 0, "coach_chats_used": 0}},
        )
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
